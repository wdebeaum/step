...
;;(defvar *skeleton* nil) ;; if set, we will filter based on these constituent boundaries
(defvar *skeleton-boost-factor* 1)
(defvar *skeleton-penalty-factor* 1)
(defvar *skeleton-constit-cats* nil)

(defun p-skeleton (x skeleton)
"Parse an input string with the parse skeletons as filter"
  (setf (skeleton *chart*) skeleton)
  (p x)
  )

(defun npfound (e)
  ;;(trace-msg 3 "~%-NP found: ~S~%" e)
  (let*
      ((newe (if (skeleton *chart*)
		 (skeleton-boost e)
	       e))
       (new2 (if *lf-prefs* (LF-preferer newe) newe)))
  (when *reference-filter-enabled*
    (call-reference-filter new2))
  new2)
  )
...
(defun skeleton-boost (e)
  (let* ((start (entry-start e))
	...
	(c (entry-constit e))
	(cat (constit-cat c)))
    (when (and (skeleton *chart*)
	       (member cat *skeleton-constit-cats*) 
	       (not (member (entry-name e) *skel-boost-constits-seen-already*))   ;; don't boost'penalize constits twice
	       (not (get-value c 'w::coerced)) )   ;; we don't boost coerced constits, as they are already boosted!
      (trace-msg 3 "~%~%CHECKING cat ~s from ~S to ~S" cat start end)
      (setq *skel-boost-constits-seen-already* (cons (entry-name e) *skel-boost-constits-seen-already*))
      (if (some #'(lambda (s) (and (member cat (car s))
				   (eq start (cadr s)) 
				   (eq end (caddr s))))
			  (skeleton *chart*))
	  (progn
...
(announce '(W::S) #'skeleton-boost)

(announce '(W::CP) #'skeleton-boost)
...
(defun parse-end-conditions-check-out (entry)
  ...
  (and (stopwhensolutionsfound *chart*)   ;; control variable that can be disable to force the parser to find all parses
	   ...
	   (>= (incr-number-found) (+ (number-parses-to-find *chart*) 1)) ;; we always look for one more to provide robustness for subsequent chart search
	   )
  )
