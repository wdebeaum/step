...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ;; gf: 20080516: openmcl apparently no longer defines :mcl
...
;; gf: 20061120: Added cmu
;; gf: sometime: Must have added abcl
;; gf: 20080516: Added openmcl/x86_64 (apparently wasn't needed for old openmcl)
#+(or clisp cormanlisp ecl (and gcl defpackage) sbcl cmu abcl (and :openmcl :x86_64))
(defpackage "MAKE" (:use "COMMON-LISP") (:nicknames "MK"))

;; gf: 20061120: Removed :nicknames which gets `old in-package' warning in cmucl
...
(defvar *dont-redefine-require*
  ...
  #+(or clisp sbcl abcl) t
  #+allegro t
  #-(or cmu sbcl abcl clisp allegro) nil
...
(defvar *central-registry*
  `(;; Current directory
    ...
    ;; gf: sometime: Must have added abcl
    #+(or :sbcl :abcl)     (progn *default-pathname-defaults*)
...
;        #+(and :mips :lispworks)             ("lisp" . "mfasl")
	 ;; gf: 20080516: openmcl apparently no longer defines :mcl
	 ;;               but this shouldn't matter since we do it in
	 ;;               ../defsystem.lisp (for some reason)
         #+(or :mcl :openmcl)                 ("lisp" . ,(pathname-type (compile-file-pathname "foo.lisp")))
         #+:coral                             ("lisp" . "fasl")
...
(defun new-append-directories (absolute-dir relative-dir)
  ...
  ;; gf: 20061120: Added :cmu to all instances of `#+(or :MCL :sbcl :clisp)'
  ;;               which seems needed at least for cmucl-19d
  ;; gf: 20080516: openmcl apparently no longer defines :mcl
  ...
  (let* ((abs-dir (pathname absolute-dir))
	 ...
	 ;; rel-name and rel-type work for every Lisp?
         #-(or :MCL :openmcl :sbcl :clisp :cmu) (rel-file (file-namestring rel-dir))
	 ;; Stig (July 2001);
	 ;; These values seems to help clisp as well
	 #+(or :MCL :openmcl :sbcl :clisp :cmu) (rel-name (pathname-name rel-dir))
	 #+(or :MCL :openmcl :sbcl :clisp :cmu) (rel-type (pathname-type rel-dir))
	 (directory nil))
    ...
    (namestring
     (make-pathname :host host
		    ...
		    :name
		    ;; gf: 20080516: openmcl apparently no longer defines :mcl
		    #-(or :sbcl :MCL :openmcl :clisp :cmu) rel-file
		    #+(or :sbcl :MCL :openmcl :clisp :cmu) rel-name

		    #+(or :sbcl :MCL :openmcl :clisp :cmu) :type
		    #+(or :sbcl :MCL :openmcl :clisp :cmu) rel-type
		    ))))
...
(unless *old-require*
  (setf *old-require*
	(symbol-function
	 ;; gf: 20080516: openmcl apparently no longer defines :mcl
	 #-(or (and :excl :allegro-v4.0) :mcl :openmcl :sbcl :abcl :lispworks) 'lisp:require
	 #+(and :excl :allegro-v4.0) 'cltl1:require
	 ;; gf: sometime: Must have added abcl
	 #+(or :sbcl :abcl) 'cl:require
