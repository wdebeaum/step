...
;;;; $Id: internal.lisp,v 1.2 2010/01/18 19:07:48 ferguson Exp $
;;;;
...
(defun facilitator-register (msg)
  ...
  (let ((name (string-upcase (string (get-keyword-arg msg :name))))
	(client nil))
    (cond ((null name)
	  ...
	  (t
	   (let* ((current-process (get-current-process))
...
posted to the bulletin board."
  ...
  (let* ((content (convert-to-package (get-keyword-arg msg :content) *comm-package*))
...
(defun facilitator-send-to-subscribers (msg)
  ...
  (let ((comm-msg (convert-to-package msg *comm-package*)))
    (facilitator-broadcast-atomically-to-clients
     msg
     (mapcan #'(lambda (sub)
		 (let ((pat (car sub))
		       (client (cdr sub)))
		   (when (match-perf-pattern pat comm-msg)
		     (list client))))
	     *subscriptions*))))
