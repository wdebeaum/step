...
sub domainSpecificInfo2trips {
  ...
  if (ref($info) eq 'ARRAY') {
	...
	if (exists($info->{score}));
      ...
      for my $key (qw(input matched via status source)) {
	push @$trips, ':' . $key,
	...
	next if (grep { $_ eq $key } qw(type input matched via status source score maybe-depluralized surely-depluralized depluralization-score corrected dash-no-dash no-dash-dash exact));
	push @$trips, ':' . $key, $info->{$key}
	  if (exists($info->{$key}));
      }
