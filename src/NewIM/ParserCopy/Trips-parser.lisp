;; THIS IS AN EXACT COPY OF Trips-parser.lisp
...
(defun parse-and-extract (text &optional cost-table)
...
(defun Parse (content)
  ...
  (case (car content)
    ...
    (word 
     ...
     (if  (and (> (length (cadr content)) 1)  ;;a string with more than one character
	  ...
	  (mapcar #'add-to-agenda 
		  (flatten
		   (mapcar #'process-word-entry (prepare-input-for-parser content)))))
     nil
     )
    ;; STARTED SPEAKING
    ...
    ((utterance end-sentence)
     ...
     (continue-BU-parse)
     (let ((utts (get-parse :lf :numb (number-parses-desired *chart*))))
       (if *include-parse-tree-in-messages*
	   (let ((syntax (get-parse :syntax :numb (number-parses-desired *chart*) :feats *include-parse-tree-in-messages*)))
...
(defun prepare-input-for-parser (in)
  (remove-if #'null 
	     (prepare-input-for-parser-one in))
...
(defun prepare-input-for-parser-one (in)
  ...
  (let* ((new-tokens   (tokenize (second in)))
	 ...
	 ;;(alternatives (find-value in :alternatives))
...
(defun parse-to-lfs (text)
  ...
  (let* ((result (parse-text (if (stringp text) (remove-if-not #'graphic-char-p text)
					 text))))
    (if (eq (car result) 'COMPOUND-COMMUNICATION-ACT)
