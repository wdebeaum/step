...
;;;
;;; Time-stamp: <Fri Feb  6 23:48:47 CST 2015 lgalescu>
...
(defun return-lf (&rest args)
  ...
  (let* ((*showlf* nil)
	 ...
	 (result (sort-lfs root (if *kr-type-info-desired* (extract-LFs+wnsense ans *kr-type-indo-desired t)
				    (extract-lfs ans)))))
    (show-lfs result))
  )
		...
		nil)
	      ((and (symbolp (first c)) (second c))  ;; atomic role name and non null value	       
               ...
               (t (parser-warn "Bad constraint list expression found: ~S Is it a list? ~S" c (consp c))
                        c))))
	  (collapse-multiple-features answer :MODS))
      (when c
...
#||(defun split-out-time-loc-paths (cs)
  "Separates out time, location and path modifiers from a list of constraints"
  (let ((times nil)
        (locs nil)
        (paths nil)
        (others nil))
    (mapc #'(lambda (con)
              (let ((c (car con)))
              (if (and (consp c) (eq (car c) :*))
                (let ((class (second c)))
                  (cond
                   ((subtype 'w::SEM class 'ONT::temporal-predicate)
                    (setq times (cons con times)))
                   ((eq class 'ONT::SPATIAL-LOC)
                    (setq locs (cons con locs)))
                   ((subtype 'w::SEM class 'ONT::TRAJECTORY)
                    (setq paths (cons con paths)))
                   (t (setq others (cons con others)))))
                (setq others (cons con others)))))
          cs)
    (values times locs paths others)))||#
...
(defvar *parser-package* (find-package "IM"))
...
(setq *role-mapping-table*
      '(
	(ont::send
	 ((ont::to-loc ont::goal-reln) :goal)
	 ((ont::position-reln) :at-loc))
	(ont::motion
	 ((ont::to-loc ont::position-reln ont::goal-reln) :goal)
	...
	(ont::change
	  ((ont::to-loc ont::goal-reln) :result)
	  ((ont::from-loc ont::from) :source))
	(ont::phys-object 
	 ((ont::position-reln ) :at-loc)
	...
	(ont::situation-root
	 ...
	 ((ont::manner) :manner)
	 ((ont::assoc-with) :assoc-with)
	 ((ont::temporal-location) :at-time)
	 ((ont::event-duration-modifier) :duration)
	 ((ont::instrument) :instrument)
	 ((ont::evaluation-attribute-val) :evaluation)
	 ((ont::pos-condition) :condition)
	 ;;((ont::goal-reln) :goal)
	 ((ont::position-reln ) :at-loc)
	 ((ont::accompaniment) :partner)
	 ((ont::by-means-of) :by-means-of)
